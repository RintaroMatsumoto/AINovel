"""翼20歳ホストv3: センターパート・ムキムキ・鋭い眼光 5枚 FaceID"""
...
NEG = ("EasyNegative, (worst quality:1.2), (low quality:2), (normal quality:2), "
       ...
       "thin, skinny, frail, weak, slender, lean, "
...
SCENES = [
    "standing in host club vip area, tall 180cm, extremely muscular broad shoulders, thick chest, big muscular arms, wearing designer suit jacket open, white shirt unbuttoned at top, silver necklace, long black hair center parted, sharp piercing eyes looking at camera, confident arrogant smirk, purple neon lights, luxury interior",
    "sitting at host club table, leaning back in chair, legs apart, muscular build visible through suit, open collar shirt, long black hair center part, sharp intense eyes staring down, half smile, champagne, dim lighting",
    "close-up portrait, long black hair center parted, sharp dangerous eyes, strong jawline, muscular neck and shoulders visible, expensive suit jacket, silver chain, cold confident expression, club background bokeh",
    "standing at host club bar, leaning on counter, full body, extremely muscular build stretching suit fabric, long hair center part, sideways glance with sharp eyes, drink in hand, confident smirk, luxury bar",
    "walking through host club floor, tall muscular figure, suit jacket over shoulder, white shirt, long black hair center parted, looking back over shoulder with piercing eyes, arrogant stride, neon lights",
]

def gen_one(prompt, prefix, i):
    wf = {
                    ...
                    for nid, node in h[pid]["outputs"].items():
                        for img in node.get("images",[]):
                            ...
                            if len(resp.content) > 1000:
                                with open(os.path.join(OUT, img["filename"]),"wb") as f: f.write(resp.content)
...
print("\n=== 20歳v3 ホスト センターパート・ムキムキ ===")
for i, scene in enumerate(SCENES, 1):
    prefix = f"tsubasa_20_host_v3_fid08_s{random.randint(100000000,999999999)}_{i:02d}"
    gen_one(scene, prefix, i)
    time.sleep(0.3)
