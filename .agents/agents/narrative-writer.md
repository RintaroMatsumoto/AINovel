...
description: |
  叙事文本创作与去AI味专家。负责正文写作（三層織り込み、感知/反应）、
...
| 参考文件 | 何时读取 |
|---|---|
| `story-setup/references/agent-references/writing-craft.md` | 正文写作（三層織り込み、身体细节、物件三次出现、小节密度）时 |
...
2. **展开子事件**：每个子事件将发生、感知、反应三層織り込み同一段连续正文（详写的子事件合计 ≥100-150 字；过场/连接类 1-2 句带过，不要每个子事件平均用力，见 writing-craft.md「疏密分配」）
   ...
   - 子事件之间用身体动作连接（~20 字）
   - **画面分段**：三層織り込み不等于一段到底。按新动作/新物件/新信息/新对话断段，不按维度断段
...
- **避免信息过载**：三層織り込み后不要一段到底；读起来卡、逗号串太长或多个完整动作挤在一段里时，按新动作/新物件/新信息/新对话拆分
...
| 禁止章末预告 | 硬 | 禁令优先 |
| 字数下限 | 硬 | 字数下限优先 |
| 三層織り込み（感知/反应/暗线） | 默认软 | 文风可调密度，但不取消織り込み |
