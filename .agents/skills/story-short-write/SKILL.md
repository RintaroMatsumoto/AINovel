...
构思阶段，如果项目已部署 story-architect agent（检查 `.opencode/agents/story-architect.md` 是否存在），可 spawn `Agent(subagent_type: "story-architect", prompt: "项目目录：{dir}\n任务类型：短篇构思\n查询参数：{情绪目标+题材方向}")` 辅助框架设计。如 agent 不可用，由主线程直接执行。
...
设计任务完成后，如果项目已部署 character-designer agent（检查 `.opencode/agents/character-designer.md` 是否存在），可 spawn `Agent(subagent_type: "character-designer", prompt: "项目目录：{dir}\n任务类型：角色设定\n查询参数：{人设速写+关系}")` 辅助角色设定和语言风格档案。如 agent 不可用，由主线程直接执行。
...
**节长达标流程**：
1. **写作时**：按三層織り込み写每个子事件——发生、感知、反应織り込み同一段连续正文，不按维度分段写
...
每个小节按「三層織り込み」写作（详见 writing-craft.md 第 8 节）：每个子事件将发生、感知、反应三个维度織り込み同一段连续正文，子事件合计 ≥150 字。维度織り込み不等于按维度分段——禁止"先写发生再补感知再补反应"的堆叠写法；也不等于一段到底，按新动作/新物件/新信息/新对话断段。长度只是诊断，先判断是否完整戏剧单元；混入多个动作/信息才拆，完整推理、氛围或情绪链可以保留稍长段。

**写完后对照 小节大纲.md 检查**：每个子事件三个维度都織り込み了？本节情绪到位？伏笔/物件已植入？节长 <800 字 → 补充更多子事件/对话后再写下一节。
...
- 环境细节只能織り込み角色的动作和感知中自然带出，不能独立成句；例外题材中，环境也必须携带威胁、异常或信息差
...
| 格式与节奏 | **`references/format-and-structure.md`**（正文格式硬规范） | `references/writing-craft.md`（三層織り込み）· `references/writing-workflow.md`（设计/精修工作流） |
