...
3. **関連サポート資料の読み取り**：本文、関連設定、キャラクターファイル、大綱、追踪/コンテキスト、伏線ファイル；欠落時はレポートで証拠不足をマーク。
...
| 对标書の再分析が必要 | story-long-analyze / story-short-analyze | `/story-long-analyze` または `/story-short-analyze` |
