...
**日更续写保持在 workflow 内**：一旦本次请求路由到 `references/workflow-daily.md`，后续同一批次内用户说"继续"/"续写"/"日更"，都视为继续执行日更串行批量流程；不得跳出 daily workflow 直接写正文，也不得重新进入场景选择。正常批量执行中不询问"是否继续"；只有細綱缺失、章节号冲突、用户明确要求逐章确认，或请求会改变既有大纲/追踪时才暂停确认。
...
**每章必须有一个細綱文件**（`大纲/細綱_第XXX章.md`），不允许跳章。
...
- **大纲/卷纲_第X卷.md**：每卷的爽点节奏+情绪弧线+人物弧线+伏線+反转（参考 outline-methods.md「大纲三层结构法」 + emotional-arc-design.md「六种弧线速查」 + reversal-toolkit.md「反转类型」）
...
├── 大纲/
│   ├── 大纲.md                # 全书卷级结构
│   ├── 卷纲_第一卷.md         # 每卷一个：爽点节奏+情绪弧线+人物弧线+伏線+反转
│   └── 細綱_第001章.md        # 每章一个：事件+钩子(章首/章尾/段落级)+爽点+悬念
├── 正文/
│   ├── 第001章_章名.md
│   └── ...
├── 对标/                          ← 拆文产出的结构化资产
...
- **細綱一章一个文件**：`細綱_第XXX章.md`，含钩子设计，与正文一一对应
...
1. **检查細綱**：读取 `大纲/細綱_第{N}章.md`。如果不存在，**必须先补建細綱再写正文**，不允许跳过細綱直接写作。补建时参考卷纲中本章对应的事件规划和上下文，并按新版“章节蓝图”模板补齐内容概括、情節安排、人物关系/出場順序、情節細化、結末設定；旧版細綱缺这些字段不阻塞读取，但本轮若要回填，未知项写 `[待补充]`。
