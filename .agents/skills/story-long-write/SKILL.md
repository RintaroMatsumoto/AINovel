...
已存在的设定文件按細綱新信息**增量补充、不覆盖**，同一角色不重复登记 `追踪/角色状态.md`。一次性路人、后文无戏份的配角不建档。建档只填細綱已确定的信息，未定字段留占位符，不提前杜撰。
...
- **追踪/伏線.md** + **追踪/时间线.md** + **追踪/角色状态.md**：伏線状态表+故事时间线+角色状态快照（参考 plot-core-methods.md「连续性追踪」、state-tracking.md「角色状态快照格式」）

前 3 章細綱额外加载 [references/opening-design.md](references/opening-design.md)（黄金三章法则+六大标准）。

#### Agent 调用：story-architect

大纲搭建阶段优先由主会话产出卷纲+首批細綱；只有结构复杂、反转链多或主会话方案不稳定时，才调用 story-architect agent。若项目已部署 story-architect agent（检查 `.claude/agents/story-architect.md` 是否存在），可 spawn `Agent(subagent_type: "story-architect", prompt: "项目目录：{dir}\n任务类型：大纲搭建\n查询参数：卷级结构+細綱+钩子/反转/情绪弧线设计")` 辅助大纲排布、钩子/反转/情绪弧线设计。如 agent 不可用，由主线程直接执行。
...
| 追踪/角色状态.md | 全书 | Phase 3 | Phase 4 每章写作前（状態選別步骤） |
...
3. **写前準備**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   - 3.1 **状態選別**：从 `追踪/角色状态.md` 中筛选本章涉及角色的当前状态，从 `追踪/伏線.md` 中筛选本章需要回收/推进的伏線。输出本节速记（参考 state-tracking.md）。如果角色状态文件不存在，从角色设定和前文推断
   - 3.2 **模块召回与文风召回**：
...
12. **更新追踪**：写完后即时更新 `追踪/伏線.md`（新增/回收伏線）、`追踪/时间线.md`（记录事件时序）和 `追踪/角色状态.md`（如本章引起角色状态变化——身份、能力、关系、公众形象——则更新对应角色条目并追加变更记录）。本章若首次引入会复用的具名角色/势力，按 Phase 3「細綱后设定补全」规则补建对应 `设定/` 档案。角色状态更新规则详见 state-tracking.md。
