...
- 本文件第 8 节「场景写法（三層織り込み）」是 Phase 3 的写作方法，format-and-structure.md 的「小节结构」是结构规范
...
| 检查项 | 偏短原因 | 补救 |
|--------|----------|------|
| 子事件三層都織り込み了？ | 只有动作没有感官锚点和身体反应 | 織り込み感官细节和身体动作（见第8节） |
| 对话只有 1-2 轮？ | 缺少对话交锋 | 加一轮权力博弈对话（见第3节） |
| 有情绪词（心痛/悲伤/愤怒）？ | 用了抽象情绪词 | 替换为身体细节（见第1节），自动增加 30-50 字 |
| 三層織り込み后仍偏短？ | 缺少对话或回忆 | 加一轮对话（第3节）或简短回忆（第7节扩展规则） |
...
三層織り込み之前，先定叙述姿态。默认锁死主视角角色的"此刻感知"——只写他此刻看到、听到、闻到、身体感到、脑中闪过的东西。这一条同时解决两个最顽固的 AI 味：代入感不足，和"作者在场"的说教/上帝腔。
...
每个子事件不是一句话就完，要包含三个维度：发生了什么、主角注意到什么、身体怎么回应。三个维度同时織り込み同一段连续正文，不按维度分段写。
...
三層織り込み不是把一个子事件塞进一个长段。織り込み解决"同一瞬间被拆成发生/感知/反应三段"的问题；断段解决"手机阅读时一段过密"的问题。
...
| 过场/赶路/信息交代/时间跳转 | 疏（略写） | 1-2 句带过，甚至一句概括，不展开三層 |
