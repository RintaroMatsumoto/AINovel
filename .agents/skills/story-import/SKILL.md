...
- 检测 `.opencode/agents/chapter-extractor.md` 是否存在（Phase 2 长篇深度分析的并行 agent）。
...
- 可选验证：如果项目已部署 story-explorer agent（检查 `.opencode/agents/story-explorer.md` 是否存在），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：progress\n查询参数：导入验证")` 交叉验证迁移数据完整性
