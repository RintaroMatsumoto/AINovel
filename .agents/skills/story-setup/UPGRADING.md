...
- 更新 narrative-writer 场景写法：使用”三層織り込み”并按画面分段控制段落密度
...
- **細綱を章設計図にアップグレード（issues #162）**：長編の `大綱/細綱_第XXX章.md` を新規作成・補完する際、旧フィールドに加えて内容概括（起因/展開/転換/山場/結末）、プロット構成（主線/副線/イベント線/感情線/論理線）、人物関係と登場順、プロット詳細化、結末設定とフックを追加；旧版細綱はそのまま續寫可能で、欠落フィールドはブロックせず、未知項目は `[未確定]` と記入。
