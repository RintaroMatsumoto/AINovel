...
- **agent 未部署**：`.opencode/agents/chapter-extractor.md` 不存在。`.opencode/agents/` 通常不随仓库提交，由 `/story-setup` 部署；模板源在 `skills/story-setup/references/templates/agents/chapter-extractor.md`，必要时可手动复制部署。
